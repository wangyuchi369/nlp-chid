    ...
    def preprocess_function_resize(examples):
        ...
        for i in range(len(examples['content'])):
            ...
            for j in range(examples['realCount'][i]):
                ...
                candidate_string = " ".join(examples['candidates'][i][j])
                idx = text.find(idiom_tag, idx+1)                                 
                return_dic['content'].append(candidate_string + " " + text[:idx] + tokenizer.mask_token*4 + text[idx+len(idiom_tag):])
    ...
    if training_args.do_train:
        ...
        with training_args.main_process_first(desc="train dataset map pre-processing"):
            train_dataset = train_dataset.map(
                ...
                batched=True,
                remove_columns=["groundTruth", "realCount", "dict"],
