...
"""
Fine-tuning the library models for CHID_dataset.
"""
...
def main():
    ...
    def preprocess_function_tokenize(examples):
        ...
        tokenized_candidates = [[tokenizer.convert_tokens_to_ids(list(candidate)) for candidate in candidates] for
                                candidates in examples['candidates']]
    ...
    def compute_metrics(eval_predictions):
        ...
        preds = np.argmax(predictions, axis=1)  # labels_id preds为(24948,)
    ...
    # %%
    output.metrics["test_samples"] = len(test_dataset)
    import pandas as pd
    trainer.log_metrics("test", output.metrics)
    trainer.save_metrics("test", output.metrics)
    preds = np.argmax(output.predictions, axis=1)
    wrong_idx = np.argwhere(preds != output.label_ids)
    res = []
    for idx in wrong_idx:
        idx = int(idx)
        item = test_dataset[idx]
        orginal = item['content']
        assert item['labels'] == output.label_ids[idx]
        gt = item['candidates'][item['labels']]
        pred_ids = item['candidates'][int(preds[idx])]
        GT = tokenizer.decode(gt).replace(' ', '')
        PRED = tokenizer.decode(pred_ids).replace(' ', '')
        res.append((orginal, GT, PRED))
    csv = pd.DataFrame(res)
    csv_file = csv.to_csv('case.csv', encoding='utf_8_sig')
    # kwargs = dict(
